...
export default function HomeScreen() {
    ...
    const { products, getProductsByStatus, deleteProduct, categories } = useProductStore();
    const { setAddModalOpen, setEditingProduct, setDefaultLocationId, setAddSpaceModalOpen, setAddSpaceParentId } = useUIStore();

    const [filter, setFilter] = useState<'all' | 'safe' | 'expiring-soon' | 'expired'>('expiring-soon');
    ...
    const filteredProducts = useMemo(() => {
        ...
        return getProductsByStatus(filter);
    }, [filter, products, getProductsByStatus]);
    ...
    const getSectionTitle = () => {
        switch (filter) {
            case 'all': return t('yourInventory'); // reusing roughly or add specific keys
            case 'safe': return t('yourInventory') + ' (Safe)';
            case 'expiring-soon': return t('expiringSoon');
            case 'expired': return t('expired');
            default: return t('yourInventory');
        }
    };
            ...
            <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 100 }}>
                ...
                <View style={styles.section}>
                    <View style={styles.statsGrid}>
                        <TouchableOpacity onPress={() => setFilter('all')} activeOpacity={0.8} style={styles.statTouch}>
                            <GradientStatCard
                        ...
                        <TouchableOpacity onPress={() => setFilter('safe')} activeOpacity={0.8} style={styles.statTouch}>
                            <GradientStatCard
                        ...
                        <TouchableOpacity onPress={() => setFilter('expiring-soon')} activeOpacity={0.8} style={styles.statTouch}>
                            <GradientStatCard
                                ...
                                isAlarm
                                isActive={filter === 'expiring-soon'}
                                styles={styles}
                            />
                        </TouchableOpacity>
                        <TouchableOpacity onPress={() => setFilter('expired')} activeOpacity={0.8} style={styles.statTouch}>
                            <GradientStatCard
                ...
                <View style={styles.section}>
                    ...
                    {filteredProducts.length === 0 ? (
                        <Text style={{ color: colors.muted[isDark ? 'dark' : 'light'], marginTop: 8 }}>
                            ...
                        </Text>
                    ) : (
...
const getStyles = (theme: 'light' | 'dark') => StyleSheet.create({
    ...
    sectionTitle: { fontSize: 18, fontWeight: 'bold', color: colors.foreground[theme], marginBottom: 12 },
